...
class BrachistochroneEnv(Env):
    def __init__(self, x_start=0, x_end=np.pi, y_start=0, y_end=-2, step_per_eps=1000,
        ...
        self.R = 1
        self.optimal_t = calculate_optimal_time(1, np.pi)  # valid for start=(0, 0), end=(pi, -2)
    ...
    def step(self, action):
        ...
        traversal_time = calculate_traversal_time(self.state, self.sample_width, self.g)
        ...
        if traversal_time == np.inf:
            ...
            for idx in range(1, len(self.state) - 1):
                curr_traversial = calculate_traversal_time(self.state[idx:], self.sample_width, self.g)
    ...
    def render(self, pause_time=.01):
        ...
        self.ln.set_data(calculate_brachistochrone(self.R, self.y_end, 50)[0], np.linspace(self.y_start, self.y_end, 50))
        self.ln.set_color("r")
        self.ln.set_linestyle("--")
        self.ax.draw_artist(self.ln)
...
##### PHYSICS #####
def calculate_optimal_time(R, t):
    """
