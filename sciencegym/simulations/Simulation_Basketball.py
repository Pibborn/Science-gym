...
class Sim_Basketball(EnvironmentInterface):
    ...
    def set_intial_angle(self, action):
        self.initial_angle = math.atan2(float(action[1]), float(action[0]))
    
    def set_initial_speed(self, action):
        self.initial_speed = b2Vec2(float(action[0]), float(action[1])).length

    # Keep the initial velocity and the initial angle
    def step_injection(self, action):
        self.set_intial_angle(action)
        self.set_initial_speed(action)
